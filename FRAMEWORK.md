...
- **直出 Blender(2026-09-06,用户"没必要先转成 Unity 文件再导入")**:UE 包不再经 Unity 资产/AR 管线/序列化工程/文本解析进 Blender。五个数据集就是全部输入(`Ruri.FModelHook.Unreal.UnrealDatasets`,参数都可写引擎对象路径或挂载文件路径,`PackageKey` 一次归一;⚠ 光 `FixPath` 不够——它在剥对象名**之前**取叶子,于是永远不补回刚被剥掉的扩展名):
